...
Project videos load when a visitor presses **Watch demo**. Preview images are still frames from the existing recordings. The playground loads only when opened. The original solid, beveled 3D logo is the default. The icon-only layers button toggles translucent logo outlines with a highlighted depth slice. Both representations share their geometry dimensions, object rotation, camera, and orbit controls, so switching preserves the viewing angle, drag inertia, and spin phase. Dragging orbits the view, and only the slider changes the slice position. The portal button still loads the portal gun using Three.js's TDS loader and local color and normal textures. Asset provenance is recorded in `public/models/portalgun/SOURCE.md`.

Both logo views and the portal gun rotate gently when visible and use damped orbit controls for drag inertia. A pause button stops their idle spin; the logo views share the pause state. Reduced motion disables automatic rotation and damping. Rendering returns to demand mode while paused, offscreen, or in a hidden tab.
