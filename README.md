...
Project videos load when a visitor presses **Watch demo**. Preview images are still frames from the existing recordings. The playground loads only when opened. The original solid, beveled 3D logo is the default. **Slice view** switches to translucent logo outlines with a highlighted depth slice: dragging orbits the view, and only the slider changes the slice position. **Solid logo** returns to the original. The portal button still loads the portal gun using Three.js's TDS loader and local color and normal textures. Asset provenance is recorded in `public/models/portalgun/SOURCE.md`.

The solid logo and portal gun rotate gently when visible and use damped orbit controls for drag inertia. A pause button stops their idle spin; reduced motion disables automatic rotation and damping. The slice view has no idle rotation and renders on demand. Rendering also returns to demand mode while paused, offscreen, or in a hidden tab.
