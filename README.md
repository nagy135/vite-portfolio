...
Project videos load when a visitor presses **Watch demo**. Preview images are still frames from the existing recordings. The playground loads only when opened. The default 3D logo is scanned along a fixed viewport direction: 45° horizontal and 20° vertical. Dragging (or using the arrow keys on the canvas) rotates the solid object, while the camera and scan direction stay fixed. The slider moves the scan plane through a fixed travel range. Every contour is an actual plane/prism intersection, recalculated as the logo rotates; translucent neighboring sections reveal the surrounding volume. The slider supports touch and keyboard input. A button loads the portal gun using Three.js's TDS loader and local color and normal textures. Asset provenance is recorded in `public/models/portalgun/SOURCE.md`.
